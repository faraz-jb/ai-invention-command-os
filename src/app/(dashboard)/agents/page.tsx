...
export default function AgentsPage() {
  ...
  const myAgents = agents.filter((a) => !a.client_id);
  const clientAgents = agents.filter((a) => a.client_id);

  function renderCard(a: AgentRow) {
    return (
      <div key={a.id} className="bg-surface border border-border rounded-xl p-4 space-y-3">
        <button className="w-full text-left space-y-3" onClick={() => toggleExpand(a.id)}>
          ...
          {a.client_name && (
            <Badge tone="internal">{a.client_name}</Badge>
          )}
      ...
      {loading ? (
        ...
        <>
          <div className="space-y-3">
            <h2 className="text-lg font-semibold">🛰️ My Agents</h2>
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {myAgents.map(renderCard)}
              {myAgents.length === 0 && <p className="text-text-dim text-sm">No internal agents yet.</p>}
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {clientAgents.map(renderCard)}
              {clientAgents.length === 0 && <p className="text-text-dim text-sm">No client agents yet.</p>}
            </div>
          </div>
        </>
      )}
